...
@override_settings(MEDIA_ROOT=TemporaryDirectory().name)
class CrudRenderLineTemplateDetailViewTestCase(APITestCase):
    def setUp(self):
        self.crud_view = factories.CrudViewFactory(name="Line", order=0,
                                                   layer__schema=json.load(open(LAYER_SCHEMA)),
    ...
    def test_style_mblg_renderer_line_with_extra_layers_with_style(self):
        ...
        extra_layer = LayerExtraGeom.objects.create(layer=self.crud_view.layer)
        FeatureExtraGeom.objects.create(feature=self.feature, layer_extra_geom=extra_layer,
                                        geom=LineString((-0.1, 44.2), (-0.2, 44.1)))
        models.ExtraLayerStyle.objects.create(layer_extra_geom=extra_layer, crud_view=self.crud_view,
                                              map_style=custom_style)
        response = self.client.get(
            reverse(
                'render-template',
                ...
                {"TMP_MBGL_BASEMAP": {"type": "raster",
                 ...
                 "primary": {"type": "geojson",
                             "data": {"type": "LineString", "coordinates": [[-0.246322800072846, 44.5562461167907],
                                                                            [0.0, 44.0]]}},
                 extra_layer.name: {"type": "geojson",
                                    "data": {"type": "LineString", "coordinates": [[-0.1, 44.2], [-0.2, 44.1]]}}},
            "layers": [
                {"id": "TMP_MBGL_BASEMAP", "type": "raster", "source": "TMP_MBGL_BASEMAP"},
                {"type": "line", "paint": {"line-color": "blue", "line-width": 5},
                 "id": extra_layer.name, "source": extra_layer.name},
                {"type": "line", "paint": {"line-color": "#000", "line-width": 3},
                 "id": "primary", "source": "primary"}]
        }
